...
const BreakTime = () => {
  ...
  const TimerCheckIn = async () => {
    setRunning1(true);
    const startTime = new Date().toISOString();

    try {
      const response = await axios.post("http://localhost:3006/timercheckin", {
        userId,
        startTime,
      });
  ...
  const TimerCheckOut = async () => {
    setRunning1(false);
    const endTime = new Date().toISOString();

    try {
      await axios.put(`http://localhost:3006/timercheckout/${userId}`, {
        endTime,
        userId,
      });
      setRecordId1(null);
    ...
    <div className="container">
      <div className="timer-group">
        ...
        <div className="actions">
          <button className="button" onClick={TimerCheckIn}>
            Cofee Break
          </button>
          <button className="button" onClick={TimerCheckOut}>
            Check Out
          </button>
        </div>
      </div>
